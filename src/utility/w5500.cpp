...
#error ARDUINO_ARCH_AVR defined 
    initSS();
    SPI.begin();
#else
#if defined(__PIC32MX__)
    SPI.begin();
//  SPI.setClockDivider(SPI_CLOCK_DIV2);	// 8.0 Mhz
    SPI.setClockDivider(SPI_CLOCK_DIV4);	// 4.0 Mhz
//  SPI.setClockDivider(SPI_CLOCK_DIV8);	// 2.0 Mhz
//  SPI.setClockDivider(SPI_CLOCK_DIV16);	// 1.0 Mhz
//  SPI.setClockDivider(SPI_CLOCK_DIV32);	// 500 Khz
//  SPI.setClockDivider(SPI_CLOCK_DIV64);	// 250 Khz
//  SPI.setClockDivider(SPI_CLOCK_DIV128);	// 125 Khz
    SPI.setDataMode(SPI_MODE0);
#else
#error PIC32MX not defined
    SPI.begin(SPI_CS);
