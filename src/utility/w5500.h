...
private:
  ...
  static void write(uint16_t _addr, uint8_t _cb, uint8_t _data);
  static void write(uint16_t _addr, uint8_t _cb, const uint8_t *buf, uint16_t len);
#else
...
private:
  ...
  static inline void     writeSn(SOCKET _s, uint16_t _addr, uint8_t *_buf, uint16_t len);
#else
...
#define __SOCKET_REGISTER_N(name, address, size)             \
  static void write##name(SOCKET _s, uint8_t *_buff) {       \
...
#if defined(__PIC32MX__)
void W5500Class::writeSn(SOCKET _s, uint16_t _addr, uint8_t _data) {
...
#if defined(__PIC32MX__)
void W5500Class::writeSn(SOCKET _s, uint16_t _addr, uint8_t *_buf, uint16_t _len) {
    uint8_t cntl_byte = (_s<<5)+0x0C;
    write(_addr, cntl_byte, _buf, _len);
}
#else
